  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/users/add', {
        ...
        body: JSON.stringify({ name, phoneNumber, fromDate, toDate }),
      });
      const data = await res.json();
      if (!res.ok) { setError(data.error || 'Failed to add user'); setLoading(false); return; }
      onUserAdded();
    } catch {
      ...
      <div style={{
        ...
        <form onSubmit={handleSubmit}>
          <div style={{ padding: '1.25rem 1.5rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            <Field label="Name" value={name} onChange={(e) => setName(e.target.value)} placeholder="John Doe" disabled={loading} required />
            <Field
              label="Phone number"
              type="tel"
              value={phoneNumber}
